...
void indicate_normal(void);
void indicate_defect(void);
...
int main(void)
{
  ...
  HAL_TIM_Base_Start(&htim3); // 마이크로초 딜레이용 타이머 시작
  HAL_UART_Receive_IT(&huart2, &rxByte, 1);  //첫 바이트 수신 대기 시작
  /* USER CODE END 2 */
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, Green_LED_Pin|Red_LED_Pin|DHT_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : Green_LED_Pin Red_LED_Pin DHT_Pin */
  GPIO_InitStruct.Pin = Green_LED_Pin|Red_LED_Pin|DHT_Pin;
...
void handle_command(const char *cmd)
{
	if(strcmp(cmd, "R") == 0)
	{
		...
		{
		    snprintf(response, sizeof(response), "T:%.1f,H:%.1f\n", temperature, humidity);
		}
		else
		{
		    snprintf(response, sizeof(response), "T:ERR,H:ERR\n");
		}
  ...
  else if(strcmp(cmd, "S") == 0)
  {
    ...
    HAL_UART_Transmit(&huart2, (uint8_t*)doneMsg, strlen(doneMsg), 100);
  }
}
