...
# --- 온습도 기반 임시 threshold 하향 조정 ---
# 판정할 때마다 아래 BASE_THRESHOLDS(고정 기준값)를 절대 직접 수정하지 않고,
# compute_request_thresholds()가 매 요청마다 새 dict를 복사해서 그 복사본만 조정한다.
# 그 복사본은 classify_defect() 함수가 끝나면 사라지는 지역변수이므로,
# 다음 판정은 항상 BASE_THRESHOLDS(원래 값)에서 다시 시작한다 -> 별도 "복원" 코드가 필요 없음.
BASE_THRESHOLDS = {"pinhole": THRESHOLD, "peeling": THRESHOLD, "crack": THRESHOLD}

THRESHOLD_DROP = 0.1                  # 조건 충족 시 낮추는 폭
PINHOLE_TEMP_LIMIT = 100.0            # 핀홀: 온도 100도 이상
CRACK_TEMP_LIMIT = 130.0              # 크랙: 온도 130도 이상
PEELING_TEMP_LIMIT = 100.0            # 박리: 온도 100도 이상 AND
PEELING_HUMIDITY_RISE_RATIO = 0.15    #        습도가 평상시 대비 15% 이상 상승
HUMIDITY_BASELINE_ALPHA = 0.10        # "평상시 습도" 이동평균 반영 비율 (10%)

_humidity_baseline_lock = threading.Lock()
_humidity_baseline = None


def _update_humidity_baseline(humidity):
    """이번 판정에 쓴 습도값은 반영하지 않고, 다음 판정을 위한 '평상시 습도' 기준선만 갱신한다.
    지수이동평균(EMA): 새 값을 10%만 반영해 기준선이 급격한 변화에 흔들리지 않고 서서히 따라가게 한다."""
    global _humidity_baseline
    if humidity is None:
        return
    with _humidity_baseline_lock:
        if _humidity_baseline is None:
            _humidity_baseline = humidity
        else:
            _humidity_baseline = (
                (1 - HUMIDITY_BASELINE_ALPHA) * _humidity_baseline
                + HUMIDITY_BASELINE_ALPHA * humidity
            )


def compute_request_thresholds(temperature, humidity):
    """이번 판정 '한 건'에만 쓸 임시 threshold dict를 만들어 반환한다.
    BASE_THRESHOLDS는 절대 수정하지 않고 매번 새로 복사하므로,
    호출이 끝나고 나면 자동으로 '원상태'가 된다."""
    thresholds = dict(BASE_THRESHOLDS)

    if temperature is None:
        return thresholds

    if temperature >= PINHOLE_TEMP_LIMIT:
        thresholds["pinhole"] -= THRESHOLD_DROP

    if temperature >= CRACK_TEMP_LIMIT:
        thresholds["crack"] -= THRESHOLD_DROP

    with _humidity_baseline_lock:
        baseline = _humidity_baseline

    humidity_spiked = (
        humidity is not None
        and baseline is not None
        and humidity >= baseline * (1 + PEELING_HUMIDITY_RISE_RATIO)
    )
    if temperature >= PEELING_TEMP_LIMIT and humidity_spiked:
        thresholds["peeling"] -= THRESHOLD_DROP
...
#-------------------------------------
#비전 AI 연결지점
def classify_defect(image_path: str, temperature=None, humidity=None):
    # 이번 판정 1건에만 쓸 임시 threshold (지역변수) - 함수가 끝나면 자동으로 사라짐
    request_thresholds = compute_request_thresholds(temperature, humidity)

    res = predict_single_image(
        image_path=image_path,
        model_path=MODEL_PATH,
        scaler_path=SCALER_PATH,
        threshold=request_thresholds,
    )
    ...
    if res["is_normal"]:
        ...
        triggered = [cls for cls, p in probs.items() if p >= request_thresholds[cls]]
        if triggered:
    ...
    for cls, p in probs.items():
        ...
        breakdown.append({
            "결함유형": kor,
            "판정": "🔴 발견" if p >= request_thresholds[cls] else "미발견",
...
def uploade():
    ...
    #판정보다 먼저 읽어야, 이번 판정에 '이번 사진과 같이 찍힌' 온습도를 반영할 수 있음
    temperature, humidity = Nucleo_bridge.read_temp_humidity()

    label, confidence, breakdown = classify_defect(filepath, temperature, humidity)

    # 이번 판정에 쓴 습도값은 반영하지 않고, '평상시 습도' 기준선만 갱신 (다음 판정용)
    _update_humidity_baseline(humidity)
